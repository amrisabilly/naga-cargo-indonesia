...
<style>
    ...
    .scrollable-container {
        max-height: 100vh; /* Set maximum height for the scrollable area */
...
<section class="h-full flex flex-col w-full py-5 overflow-hidden">
    ...
    <div class="flex-1 bg-white rounded-xl shadow-lg overflow-hidden flex flex-col">
        <div class="p-6 flex-1 flex flex-col scrollable-container"> <!-- Added scrollable-container class -->
                ...
                <table id="dataKurirTable" class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-3 py-4 text-base text-gray-900" title="{{ $kurir->nama }}">{{ $kurir->nama }}</td>
                            <td class="px-3 py-4">
                                <div class="credential-item">
                                    <span class="credential-text text-[#879FFF] font-semibold">{{ $kurir->username }}</span>
                                    <button class="copy-btn p-1 rounded text-gray-500 hover:text-[#879FFF]"
                                        onclick="copyToClipboard('{{ $kurir->username }}', this)" title="Salin Username">
                            ...
                            <td class="px-3 py-4">
                                <div class="credential-item">
                                    ...
                                    <button class="copy-btn p-1 rounded text-gray-500 hover:text-[#879FFF]"
                                        onclick="copyToClipboard('{{ $kurir->password }}', this)" title="Salin Password">
                                        <i class="bx bx-copy text-base"></i>
                                    </button>
                                </div>
                            </td>
                            <td class="px-3 py-4 text-base text-gray-900">{{ $kurir->no_hp }}</td>
                            <td class="px-3 py-4">
                                <span
                                    ...
                                    <div class="w-1.5 h-1.5 {{ $kurir->status === 'Aktif' ? 'bg-green-500' : 'bg-red-500' }} rounded-full mr-2"></div>
    ...
    <div class="relative top-20 mx-auto p-5 border w-[30em] shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 id="modalTitle" class="text-lg font-medium text-gray-900">Tambah Kurir Baru</h3>
            ...
            {{-- Form --}}
            <form id="formKurir" action="{{ route('dashboard.data-pengguna.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="noTelepon" name="no_hp"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#879FFF] focus:border-transparent"
    ...
    // Copy to clipboard function
        function copyToClipboard(text, button) {
            navigator.clipboard.writeText(text).then(function() {
                ...
                icon.className = 'bx bx-check text-sm text-green-500';
        ...
        $(document).ready(function() {
            ...
            // Edit Kurir - Load data into modal
            $(document).on('click', '.edit-kurir', function() {
                const row = $(this).closest('tr');
                const id = row.find('td:eq(0)').text();
                const nama = row.find('td:eq(1)').text();
                const username = row.find('td:eq(2) .credential-text').text();
                const password = row.find('td:eq(3) .credential-text').text();
                const no_hp = row.find('td:eq(4)').text();
                const status = row.find('td:eq(5) .rounded-full').hasClass('bg-green-500') ? 'Aktif' : 'Nonaktif';
                ...
                openModal('Edit Data Kurir');
            });

            // Delete Kurir - Hanya tampilkan alert
            $(document).on('click', '.hapus-kurir', function() {
                Swal.fire({
                    title: 'Hapus Kurir',
                    text: 'Apakah Anda yakin ingin menghapus data kurir ini?',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Berhasil!',
                            text: 'Data kurir berhasil dihapus.',
                            icon: 'success',
                            timer: 2000,
            ...
            //     Swal.fire({
            //         title: 'Berhasil!',
            //         text: `Data kurir berhasil ${isEdit ? 'diperbarui' : 'ditambahkan'}.`,
            //         icon: 'success',
            //         timer: 2000,
            //         showConfirmButton: false
            //     }).then(() => {
